...
@register.filter(name="poll_option_field")
def poll_option_field(poll_form, n):
    # Looks up PollForm's dynamically-declared option_<n> field by number, so
    # the template can render them in a loop instead of naming all ten.
    return poll_form[f"option_{n}"]
...
def group_reactions(reactions, viewer):
    # `reactions` is a post's or comment's already-prefetched .reactions.all() -
    # grouping here (not via a queryset annotation) avoids the join fan-out
    ...
    return [
        ...
        for value, glyph in Reaction.Emoji.choices
    ]
