...
│   ├── models.py                   # Post, Comment (both with an `edited` flag - see §5.5), PostVote, CommentVote
...
│   ├── forms.py                    # PostForm, CommentForm, CommentEditForm
│   ├── views.py                    # feed, following-feed, CRUD, editing, voting, annotate_votes/toggle_vote
│   ├── urls.py
│   ├── templatetags/post_extras.py # {{ body|markdown }} filter
│   ├── static/posts/js/infinite_scroll.js  # generic scroll-triggered pagination - see §5.6
│   └── templates/posts/            # feed, post_detail, post_form, _comment, _post_card/_post_list, _comment_list
...
└── templates/                      # base.html + shared partials
    ...
    ├── _header.html                # nav + theme toggle + your-turn badge (§5.7)
    ├── _avatar.html
    ├── _post_vote.html
    └── _comment_vote.html
```
