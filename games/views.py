...
# Same generous-sanity-bound approach as Doodle Jump - pipes passed has no
...
class FlappyBirdView(LoginRequiredMixin, TemplateView):
    template_name = "games/flappy_bird.html"


class FlappyBirdFinishView(LoginRequiredMixin, View):
    def post(self, request):
        try:
            data = json.loads(request.body)
            score = int(data["score"])
        ...
        if not (0 <= score <= MAX_FLAPPY_SCORE):
            return JsonResponse({"error": "invalid score"}, status=400)
